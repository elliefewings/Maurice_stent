...
#
# Script for plotting results of CellPhoneDB
# ============================================================
# Usage: (from terminal)
#
# $ ./cellphoneDB_plots.R [options]
#
# Options:
#  -i INPUT, --input=INPUT
#  Path to directory containing results of CellPhoneDB [required]
#
#  -m METADATA --metadata=METADATA
#  Path to file containing metadata supplied to CellPhoneDB [required]
#
#  -h, --help
...
## Load libraries
libs <- c("dplyr", "tidyr", "ggplot2", "optparse", "magrittr", "cowplot", "stringr", "igraph", "ComplexHeatmap", "ggraph", "networkD3", "gridExtra")
...
option_list <- list(
  make_option(c("--input", "-i"), action="store", default=NULL, type='character',
              help="Path to directory containing results of CellPhoneDB [required]"),
  make_option(c("--metadata", "-m"), action="store", default=NULL, type='character',
              help="Path to file containing metadata supplied to CellPhoneDB [required]")
)
...
if (is.null(opt$metadata)) {
  message("ERROR: Metadata missing, please specify with --metadata, -m flags.")
  stop(parse_args(OptionParser(option_list=option_list), args = c("--help")))
}
...
deconv.case <- read.table(paste(opt$input, "/deconvoluted.txt", sep=""), sep="\t", stringsAsFactors = FALSE, header=TRUE, check.names=FALSE)
means.case <- read.table(paste(opt$input, "/means.txt", sep=""), sep="\t", stringsAsFactors = FALSE, header=TRUE, check.names=FALSE)
pval.case <- read.table(paste(opt$input, "/pvalues.txt", sep=""), sep="\t", stringsAsFactors = FALSE, header=TRUE, check.names=FALSE)
smeans.case <- read.table(paste(opt$input, "/significant_means.txt", sep=""), sep="\t", stringsAsFactors = FALSE, header=TRUE, check.names=FALSE)

meta <- read.table(opt$metadata, sep="\t", stringsAsFactors = FALSE, header=TRUE, check.names=FALSE)
...
#Clean up section
rm(cnts_recievers, cnts_senders)
...
smeans.goi <- read.table(paste(opt$input, "/significant_means.txt", sep=""), sep="\t", stringsAsFactors = FALSE, header=TRUE, check.names=FALSE)
